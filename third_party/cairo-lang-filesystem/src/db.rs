...
pub struct FilesGroupInput {
    ...
    /// Keyed override slots for file content. Updating a slot keeps invalidation scoped to
    /// the affected file instead of rewriting the full overrides map.
    #[returns(ref)]
    pub keyed_file_overrides: Option<OrderedHashMap<FileInput, FileOverrideSlot>>,
...
pub fn files_group_input(db: &dyn Database) -> FilesGroupInput {
    FilesGroupInput::new(db, None, None, None, None, None, None)
}
...
pub fn init_files_group<'db>(db: &mut (dyn Database + 'db)) {
    ...
    inp.set_keyed_file_overrides(db)
...
pub fn file_overrides<'db>(db: &'db dyn Database) -> OrderedHashMap<FileId<'db>, ArcStr> {
    ...
    inp.iter()
        .map(|(file_id, content)| {
            (
                file_id.clone().into_file_long_id(db).intern(db),
                ArcStr::new(content.clone()),
            )
        })
        .collect()
}
...
macro_rules! override_file_content {
    ($self:expr, $file:expr, $content:expr) => {
        ...
        $crate::db::set_file_override_content_keyed($self, file, $content);
    };
}
...
pub fn translate_location(code_mapping: &[CodeMapping], span: TextSpan) -> Option<TextSpan> {
    ...
    let (first, matched) = matched
        .split_first()
        .expect("non-empty vec always has first element");
    ...
    for mapping in matched {
        ...
        let mapping_origin = mapping
            .origin
            .as_span()
            .expect("mappings with start origin should be filtered out");
        let last_origin = last
            .origin
            .as_span()
            .expect("mappings with start origin should be filtered out");
