...
- Source-kind classification per `items[]` row:
  ...
  For `selection.coverage=complete_deployed_contracts`, this is no longer only
  a claim guard: the inventory, source-index, and corpus validators reject
  missing `source_kind` and reject `declared_class` rows.
- Deduplication rules for repeated class hashes or reused source packages,
  recorded in `deduplication`.
- Claim wording must match the deduplication policy. A corpus deduplicated by
  `class_hash` or `source_package` can support a "selected deduped deployed
  units" claim, not an "every deployed contract address" claim. The stronger
  contract-address claim requires `deduplication.key=none` and matching input,
  deduped, and item counts; mismatched non-deduped counts are rejected before
  a benchmark artifact is produced.
