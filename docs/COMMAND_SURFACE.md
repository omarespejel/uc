...
5. `benchmarks/scripts/run_strict_supported_set_benchmarks.sh`
- Consumes a prior mixed real-repo benchmark artifact, extracts only the
  `native_supported` cases, reruns them through the same-window real-repo
  harness, and emits a strict supported-set artifact with selection provenance
  and a guarded native-supported speed claim.
- Use this wrapper for launch-speed evidence. Do not manually filter tags out of
  a mixed support-matrix report.

6. `uc session-key`
- Generates deterministic session key from normalized input fields.

7. `uc project inspect`
