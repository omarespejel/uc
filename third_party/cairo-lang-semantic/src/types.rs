...
use crate::items::generics::{GenericParamExtractionContext, GenericParamSemantic, displayable_concrete};
...
impl<'db> TypeId<'db> {
    ...
    pub(crate) fn extract_generic_params_with_ctx(
        ...
        if !extraction_ctx.visited_types.insert(*self) {
            return Ok(());
        }
        self.long(db)
    ...
    pub(crate) fn extract_generic_params_with_ctx(
        ...
        match self {
            ...
            TypeLongId::Tuple(tys) => {
                for ty in tys {
                    ty.extract_generic_params_with_ctx(db, generic_parameters, extraction_ctx)?
                }
            }
            TypeLongId::Snapshot(ty) => {
                ty.extract_generic_params_with_ctx(db, generic_parameters, extraction_ctx)?
            ...
            TypeLongId::FixedSizeArray { type_id, .. } => {
                type_id.extract_generic_params_with_ctx(db, generic_parameters, extraction_ctx)?
            }
