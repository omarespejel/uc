...
pub struct SemanticGroupInput {
    ...
    pub skip_unused_import_diagnostics: bool,
}

#[salsa::tracked(returns(ref))]
pub fn semantic_group_input(db: &dyn Database) -> SemanticGroupInput {
    SemanticGroupInput::new(db, None, None, false)
}
...
pub trait SemanticGroup: Database {
    ...
    /// Returns whether warning-only unused-import diagnostics should be skipped.
    fn skip_unused_import_diagnostics(&self) -> bool {
        semantic_group_input(self.as_dyn_database()).skip_unused_import_diagnostics(self)
    }
...
fn add_unused_item_diagnostics<'db>(
    ...
    if db.skip_unused_import_diagnostics() {
        return;
    }
...
pub trait SemanticGroupEx: Database {
    ...
    /// Controls whether warning-only unused-import diagnostics are computed.
    fn set_skip_unused_import_diagnostics(&mut self, skip: bool) {
        let db_ref = self.as_dyn_database();
        semantic_group_input(db_ref).set_skip_unused_import_diagnostics(self).to(skip);
    }
}
