...
impl CrateInput {
    ...
    pub fn into_crate_ids(
        ...
        inputs
            .into_iter()
            .map(|input| input.into_crate_long_id(db).intern(db))
            .collect()
    }
}
...
impl<'db> CrateLongId<'db> {
    ...
    pub fn core(db: &'db dyn Database) -> Self {
        CrateLongId::Real {
            name: SmolStrId::from(db, CORELIB_CRATE_NAME),
            discriminator: None,
        }
    }
...
impl<'db> VirtualFile<'db> {
    ...
    fn into_virtual_file_input(self, db: &dyn Database) -> VirtualFileInput {
        VirtualFileInput {
            parent: self.parent.map(|loc| {
                (
                    Arc::new(loc.file_id.long(db).clone().into_file_input(db)),
                    loc.span,
                )
            }),
...
pub enum DirectoryInput {
    Real(PathBuf),
    Virtual {
        files: BTreeMap<String, FileInput>,
        dirs: BTreeMap<String, Box<DirectoryInput>>,
    },
}
...
pub enum Directory<'db> {
    ...
    Virtual {
        ...
        dirs: BTreeMap<String, Box<Directory<'db>>>,
    },
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>, db: &'db dyn Database) -> std::fmt::Result {
        ...
        let starting_pos = match self.span.start.position_in_file(db, self.file_id) {
            Some(starting_text_pos) => {
                ...
                format!(
                    "{}:{}",
                    starting_text_pos.line + 1,
                    starting_text_pos.col + 1
                )
            }
            None => "?".into(),
        };
