...
#[derive(Default)]
pub(crate) struct GenericParamExtractionContext<'db> {
    pub(crate) visited_consts: UnorderedHashSet<ConstValueId<'db>>,
    pub(crate) visited_generic_args: UnorderedHashSet<GenericArgumentId<'db>>,
    pub(crate) visited_types: UnorderedHashSet<TypeId<'db>>,
}
...
impl<'db> GenericArgumentId<'db> {
    ...
    pub fn extract_generic_params(
        ...
        let mut extraction_ctx = GenericParamExtractionContext::default();
        self.extract_generic_params_with_ctx(db, generic_parameters, &mut extraction_ctx)
    }

    pub(crate) fn extract_generic_params_with_ctx(
        &self,
        db: &'db dyn Database,
        generic_parameters: &mut OrderedHashSet<GenericParamId<'db>>,
        extraction_ctx: &mut GenericParamExtractionContext<'db>,
    ) -> Maybe<()> {
        if !extraction_ctx.visited_generic_args.insert(*self) {
            return Ok(());
        }
        match self {
            GenericArgumentId::Type(ty) => ty.extract_generic_params_with_ctx(
                db,
                generic_parameters,
                extraction_ctx,
            )?,
            GenericArgumentId::Constant(const_value_id) => {
                const_value_id
                    .extract_generic_params_with_ctx(db, generic_parameters, extraction_ctx)?;
            }
            GenericArgumentId::Impl(impl_id) => {
                for garg in impl_id.concrete_trait(db)?.generic_args(db) {
                    garg.extract_generic_params_with_ctx(db, generic_parameters, extraction_ctx)?;
                }
            }
