...
Current patches add opt-in frontend size-estimation trace counters for the Cairo `2.14` helper lane. They are silent unless `UC_CAIRO214_SIZE_TRACE=/abs/path/to/trace.tsv` is set while running that helper. Trace rows use cheap Salsa ids by default; set `UC_CAIRO214_SIZE_TRACE_NAMES=1` only when bounded function previews and stable FNV-1a grouping hashes are needed.
